        ...
        if (srslte_pscch_decode(&pscch, equalized_sf_buffer, sci_rx, pscch_prb_start_idx) == SRSLTE_SUCCESS) {
          if (srslte_sci_format1_unpack(&sci, sci_rx) == SRSLTE_SUCCESS) {   // WE CARE ABOUT THIS !!!
            ...
            */ 
            ...
            for (int sch = 0; sch < total_subchannels; sch++) {
                if ((sch+1) < sub_channel_start_idx || (sch+1) > sub_channel_end_idx)
                    resource_block[sch] = false; // RB not allocated
                else 
                    resource_block[sch] = true;  // RB is allocated
            }

            // print format: <time_stamp> <subchannel_utilization>
            printf("Time Stamp: %lu \t", (uint64_t) round(srslte_timestamp_real(&ue_sync.last_timestamp) * 1e6)); // get time stamp 
            printf("Resource Blocks: ");
            for (int i = 0; i < total_subchannels; i++) {}
